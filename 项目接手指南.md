...
当前版本：`0.16.0`。项目根目录：`D:\develop\agent for obsidian`。
...
| `src/services/deepseek-client.ts` | DeepSeek 文本客户端 | `testConnection()`、`answerWithSources()`、`answerFromWeb()`、`suggestCapture()`。只处理文本问答和 JSON 笔记提案。 |
...
| `src/runtime/agent-tools.ts` | Agent 工具注册表 | 将 16 个动作映射到六类语义工具，并在启动时验证每个白名单动作都有执行器。 |
